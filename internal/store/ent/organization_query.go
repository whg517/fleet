...
func (_q *OrganizationQuery) QueryClusters() *ClusterQuery {
...
func (_q *OrganizationQuery) QueryEnvironments() *EnvironmentQuery {
...
// QueryRegistries chains the current query on the "registries" edge.
func (_q *OrganizationQuery) QueryRegistries() *RegistryQuery {
	query := (&RegistryClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (_q *OrganizationQuery) QueryConfigSnapshots() *ConfigSnapshotQuery {
...
func (_q *OrganizationQuery) WithClusters(opts ...func(*ClusterQuery)) *OrganizationQuery {
	query := (&ClusterClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *OrganizationQuery) WithEnvironments(opts ...func(*EnvironmentQuery)) *OrganizationQuery {
	query := (&EnvironmentClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
// WithRegistries tells the query-builder to eager-load the nodes that are connected to
// the "registries" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *OrganizationQuery) WithRegistries(opts ...func(*RegistryQuery)) *OrganizationQuery {
	query := (&RegistryClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *OrganizationQuery) WithConfigSnapshots(opts ...func(*ConfigSnapshotQuery)) *OrganizationQuery {
	query := (&ConfigSnapshotClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *OrganizationQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Organization, error) {
	...
	if query := _q.withClusters; query != nil {
		if err := _q.loadClusters(ctx, query, nodes,
			func(n *Organization) { n.Edges.Clusters = []*Cluster{} },
			func(n *Organization, e *Cluster) { n.Edges.Clusters = append(n.Edges.Clusters, e) }); err != nil {
			return nil, err
		}
	}
	if query := _q.withEnvironments; query != nil {
		if err := _q.loadEnvironments(ctx, query, nodes,
			func(n *Organization) { n.Edges.Environments = []*Environment{} },
			func(n *Organization, e *Environment) { n.Edges.Environments = append(n.Edges.Environments, e) }); err != nil {
			return nil, err
		}
	}
	if query := _q.withRegistries; query != nil {
...
func (_q *OrganizationQuery) loadUsers(ctx context.Context, query *UserQuery, nodes []*Organization, init func(*Organization), assign func(*Organization, *User)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(user.FieldOrgID)
	}
...
func (_q *OrganizationQuery) loadClusters(ctx context.Context, query *ClusterQuery, nodes []*Organization, init func(*Organization), assign func(*Organization, *Cluster)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[string]*Organization)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(cluster.FieldOrgID)
	}
	query.Where(predicate.Cluster(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(organization.ClustersColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (_q *OrganizationQuery) loadEnvironments(ctx context.Context, query *EnvironmentQuery, nodes []*Organization, init func(*Organization), assign func(*Organization, *Environment)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[string]*Organization)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(environment.FieldOrgID)
	}
	query.Where(predicate.Environment(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(organization.EnvironmentsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (_q *OrganizationQuery) loadRegistries(ctx context.Context, query *RegistryQuery, nodes []*Organization, init func(*Organization), assign func(*Organization, *Registry)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[string]*Organization)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(registry.FieldOrgID)
	}
...
func (_q *OrganizationQuery) loadTemplates(ctx context.Context, query *TemplateQuery, nodes []*Organization, init func(*Organization), assign func(*Organization, *Template)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(template.FieldOrgID)
	}
...
func (_q *OrganizationQuery) loadConfigSnapshots(ctx context.Context, query *ConfigSnapshotQuery, nodes []*Organization, init func(*Organization), assign func(*Organization, *ConfigSnapshot)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[string]*Organization)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(configsnapshot.FieldOrgID)
	}
