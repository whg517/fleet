...
func (_q *TemplateQuery) QueryVersions() *TemplateVersionQuery {
	query := (&TemplateVersionClient{config: _q.config}).Query()
...
func (_q *TemplateQuery) QueryOrganization() *OrganizationQuery {
...
func (_q *TemplateQuery) WithVersions(opts ...func(*TemplateVersionQuery)) *TemplateQuery {
	query := (&TemplateVersionClient{config: _q.config}).Query()
...
func (_q *TemplateQuery) WithOrganization(opts ...func(*OrganizationQuery)) *TemplateQuery {
	query := (&OrganizationClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *TemplateQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Template, error) {
	...
	if query := _q.withOrganization; query != nil {
		if err := _q.loadOrganization(ctx, query, nodes, nil,
			func(n *Template, e *Organization) { n.Edges.Organization = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *TemplateQuery) loadVersions(ctx context.Context, query *TemplateVersionQuery, nodes []*Template, init func(*Template), assign func(*Template, *TemplateVersion)) error {
...
func (_q *TemplateQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withOrganization != nil {
			_spec.Node.AddColumnOnce(template.FieldOrgID)
		}
	}
