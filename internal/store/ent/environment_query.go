...
func (_q *EnvironmentQuery) QueryCluster() *ClusterQuery {
...
func (_q *EnvironmentQuery) QueryOrganization() *OrganizationQuery {
...
func (_q *EnvironmentQuery) WithCluster(opts ...func(*ClusterQuery)) *EnvironmentQuery {
	query := (&ClusterClient{config: _q.config}).Query()
...
func (_q *EnvironmentQuery) WithOrganization(opts ...func(*OrganizationQuery)) *EnvironmentQuery {
	query := (&OrganizationClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
//		GroupBy(environment.FieldOrgID).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
func (_q *EnvironmentQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Environment, error) {
	...
	if query := _q.withCluster; query != nil {
		if err := _q.loadCluster(ctx, query, nodes, nil,
			func(n *Environment, e *Cluster) { n.Edges.Cluster = e }); err != nil {
			return nil, err
		}
	}
...
func (_q *EnvironmentQuery) loadCluster(ctx context.Context, query *ClusterQuery, nodes []*Environment, init func(*Environment), assign func(*Environment, *Cluster)) error {
...
func (_q *EnvironmentQuery) loadOrganization(ctx context.Context, query *OrganizationQuery, nodes []*Environment, init func(*Environment), assign func(*Environment, *Organization)) error {
	ids := make([]string, 0, len(nodes))
	nodeids := make(map[string][]*Environment)
	for i := range nodes {
		fk := nodes[i].OrgID
...
func (_q *EnvironmentQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withCluster != nil {
			_spec.Node.AddColumnOnce(environment.FieldClusterID)
		}
		if _q.withOrganization != nil {
			_spec.Node.AddColumnOnce(environment.FieldOrgID)
		}
	}
